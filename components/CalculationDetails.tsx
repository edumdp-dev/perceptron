...
const CodeBlock: React.FC<{ children: React.ReactNode, className?: string }> = ({ children, className }) => (
    <code className={`block bg-gray-800/80 border border-gray-700 text-cyan-300 px-3 py-2 rounded-lg text-xs mt-1 whitespace-pre-wrap ${className}`}>
        {children}
    </code>
);
...
const CalculationDetails: React.FC<CalculationDetailsProps> = ({ calculation }) => {
    ...
    return (
        <div className="bg-gray-900 border border-red-500/30 rounded-xl p-4 shadow-lg animate-fade-in">
            ...
            <div className="space-y-1 text-sm text-gray-300">
                 <p className="text-gray-400 text-xs mb-3">O modelo errou. Veja o passo a passo de como ele aprende:</p>
                
                <div>
                    <h4 className="font-semibold text-gray-300 text-xs">1. Cálculo da Soma Ponderada (z)</h4>
                ...
                <div>
                    <h4 className="font-semibold text-gray-300 text-xs">2. Função de Ativação (ŷ)</h4>
                ...
                <div>
                    <h4 className="font-semibold text-gray-300 text-xs">3. Cálculo do Erro</h4>
                    <CodeBlock className="text-red-300">{`erro = y - ŷ\nerro = ${y_actual} - ${y_predicted} = ${error}`}</CodeBlock>
                </div>

                <Arrow />
                
                <div>
                    <h4 className="font-semibold text-gray-300 text-xs">4. Atualização dos Pesos (η = {learningRate.toFixed(2)})</h4>
                ...
                <div className="pt-3 mt-3 border-t border-gray-700/50">
                    <p className="text-xs text-gray-400 italic">
                        <strong>Próximo passo:</strong> Estes novos pesos (<code className="text-cyan-300">{new_w1.toFixed(2)}</code>, <code className="text-cyan-300">{new_w2.toFixed(2)}</code>, <code className="text-cyan-300">{new_b.toFixed(2)}</code>) serão usados na próxima iteração.
                    </p>
                </div>
